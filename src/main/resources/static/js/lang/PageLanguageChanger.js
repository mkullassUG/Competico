...
*/
const PageLanguageChanger = (callbackInit, debug = false) => {
    ...
    var getAndSetupLanguage = () => {
        ...
        if ( containsAll(["tasks", "import", "global"],path)) {
    ...
    self.getTextFor = (selector, language, isOther = true) => {
        ...
        if ( !isOther ) 
            return self.data[selector][(language? language:self.langString)];
        else if ( selector != 'OTHER_PAGE_ELEMENTS') 
            return self.data.OTHER_PAGE_ELEMENTS[selector][(language? language:self.langString)];
    }

    var setupPageLanguage = (langString, pageName, data , languageText) => {
        ...
        var otherElements;
        if ( data.OTHER_PAGE_ELEMENTS )
            otherElements = data.OTHER_PAGE_ELEMENTS;

        var ObjectsToChange = Object.keys(data);
        ObjectsToChange.splice(ObjectsToChange.indexOf('OTHER_PAGE_ELEMENTS'), 1);

        if ( callbackInit )
            callbackInit(data, langString);

        $("#languageSelected").html(languageText);

        for (let i = 0; i < ObjectsToChange.length; i++) {
            var selector = ObjectsToChange[i];
            findBySelectorAndChangeTo(selector, data[selector][langString], data[selector].ATTRIBUTE, data[selector].NOT);
        }
        ...
        for ( let i = 0; i < otherElementsKeys.length; i++) {
            ...
            if ( oElemKey == 'document.title')
                document.title = oElem[langString];
        }
    ...
    var ajaxReceiveLangInfo = ( langString, pageName, setupCB) => {
        $.ajax({
            type     : "GET",
            cache    : false,
            ...
            success: function(data, textStatus, jqXHR) {
                    ...
                    setupCB(langString, pageName, data);
            },
