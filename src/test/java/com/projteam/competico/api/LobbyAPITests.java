...
public class LobbyAPITests
{
	...
	{
		String gameCode = "gameCode";
		Account acc = mockPlayer("testPlayer");
		when(lobbyService.getRandomLobby(acc)).thenReturn(gameCode);
	...
	{
		when(lobbyService.getRandomLobby(PlayerDataService.DEFAULT_RATING)).thenReturn(null);
		when(accountService.getAuthenticatedAccount())
			.thenReturn(Optional.of(mockPlayer("testPlayer")));
		mvc.perform(get("/api/v1/lobby/random"))
